...
namespace IRCTCClone.Models
{
    public class Train
    {
        public int Score { get; set; }  // AI ranking score
        ...
        public string Durations
        {
            get
            {
                ...
                return $"{(int)ts.TotalHours:00}h {ts.Minutes:00}m";
            }
        }
        ...
        public static List<Train> GetTrains(string connectionString, int fromStationId, int toStationId, string journeyDateStr, string quota)
        {
            // Parse journey date from query string or default to today
            DateTime journeyDate = DateTime.Today;

            if (!string.IsNullOrEmpty(journeyDateStr))
            {
                DateTime.TryParse(journeyDateStr, out journeyDate);
            }
            ...
            using (var conn = new SqlConnection(connectionString))
            {
                ...
                using (var cmd = new SqlCommand("spSearchTrainsCluster", conn))
                {
                    ...
                    cmd.Parameters.Add("@JourneyDate", SqlDbType.Date).Value = journeyDate;
                    
                    //cmd.Parameters.AddWithValue("@JourneyDate", journeyDateStr);
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            trains.Add(new Train
                            {
                                ...
                                HasAlerts = reader.GetInt32(reader.GetOrdinal("HasAlert")) == 1,
                                RouteDay = reader.GetInt32(reader.GetOrdinal("RouteDay")),
                ...
                {
                    train.Classes = TrainClass.GetClasses(connectionString, train.Id, train.FromStationId, train.ToStationId, journeyDate, quota);
                }
            }

            Console.WriteLine("GetTrains QUOTA: " + quota);
        ...
        public static bool CheckDuplicate(string connectionString, int number)
        {
            using (var conn = new SqlConnection(connectionString))
            {
                conn.Open();
